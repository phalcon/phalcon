...
class Version3 extends AbstractUuid
{
    ...
    {
        $hash = md5($this->namespaceToBytes($namespaceName) . $name, true);
        $hash = substr_replace(
            $hash,
            chr((ord(substr($hash, 6, 1)) & 0x0f) | 0x30),
            6,
            1
        );
        $hash = substr_replace(
            $hash,
            chr((ord(substr($hash, 8, 1)) & 0x3f) | 0x80),
            8,
            1
        );

        $this->uid = $this->format(bin2hex($hash));
    }
}
